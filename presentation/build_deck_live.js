...
function foot(s, extra) {
  s.addText((extra ? extra + "   ·   " : "") + "Команда kosmo-nizni_klodiki_148   ·   " + REPO, {
...
s.addText("видеозапись работы — 20 секунд, всё видно", {
...
huge(s, "Требование «ISL не менее 3000 км»\nбыло избыточным", 0.95, 32);
stat(s, M, 2.85, 4.6, "2725", "км — порог, выше которого\nдоступность не растет", ACCENT, 72);
s.addText("проверено с шагом 25 км\nв диапазоне 2000–3000 км", { x: M, y: 5.15, w: 4.4, h: 0.7, fontFace: F, fontSize: 13, color: MUTED, margin: 0 });
s.addText("Требование к аппаратуре снижается\nс инженерным запасом на радиолинию.", { x: M, y: 5.95, w: 4.4, h: 0.7, fontFace: F, fontSize: 13, color: WHITE, margin: 0 });
...
huge(s, "Загруженный аппарат и критичный\nаппарат — разные аппараты", 0.95, 30);
stat(s, M, 2.9, 4.0, "S44", "№1 в ранжировании критичности\n(анализ одиночного отказа):\nотказ снижает min доступность\nна 2,36 п.п., 45 клиент-отсчетов\nтеряют маршрут", ACCENT, 54);
s.addText("Самый загруженный — S45 (83 пары\n«момент × пункт») — при этом\nне самый критичный.", {
  x: M, y: 5.5, w: 4.0, h: 1.2, fontFace: F, fontSize: 13, color: MUTED, margin: 0,
});
s.addShape(pres.shapes.ROUNDED_RECTANGLE, { x: 5.15, y: 2.6, w: 7.6, h: 4.35, fill: { color: PANEL }, line: { type: "none" }, rectRadius: 0.07 });
...
const rr = [
  ...
  ["Leave-one-out за один проход", "все 48 аппаратов проверены без пересчета орбит — секунды"],
...
stat(s, M, 3.1, 3.9, "+1,67 п.п.", "минимальной доступности:\nфазирование, найденное\nоптимизатором за 40 прогонов", ACCENT, 44);
stat(s, 4.75, 3.1, 3.9, "275 км", "запаса в требовании\nк дальности ISL:\nпорог 2725, штатно 3000", WHITE, 44);
stat(s, 8.9, 3.1, 3.9, "133", "автотеста: границы, маршруты,\nэкспорт, интерфейс —\nвсе проходят", WHITE, 44);
...
const dev = [
  ...
  ["Большие группировки", "движок принимает произвольную структуру сценария; проверен на синтетике с другой геометрией"],
];
dev.forEach((r, i) => {
  const y = 2.75 + i * 1.05;
