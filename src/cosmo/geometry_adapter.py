...
def positions(scenario: dict[str, Any], t_s: float) -> tuple[list[str], np.ndarray, np.ndarray]:
    ...
    Возвращает (ids, инерциальные координаты [км], координаты в системе,
    связанной с Землёй [км]). Официальный snapshot использует Earth-fixed
    координаты при расчёте наземных и межспутниковых контактов.
    """
