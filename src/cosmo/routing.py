...
Стратегии:
A. min_hops (обязательная, default) — детерминированный BFS:
   минимальное число рёбер; при равенстве — лексикографически
   наименьший путь (обход соседей по отсортированным ID, FIFO,
   терминальный шлюз выбирается по (дистанция, порядок обнаружения, ID)).
   Первое обнаружение узла в BFS с отсортированной смежностью даёт
   лексикографически наименьший кратчайший путь, поэтому правило
   выбора корректно.
B. min_distance — релаксация по ключу (длина км, число рёбер, путь):
   детерминированный аналог Дейкстры на маленьком графе.
   Альтернативная стратегия: физически «лучше» её не утверждаем.
...
def find_route(g: LinkGraph, client_id: str, strategy: str = STRATEGY_MIN_HOPS) -> RouteResult:
